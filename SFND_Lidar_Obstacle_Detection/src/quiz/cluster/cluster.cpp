...
{
	//std::cout<< "Proximity enter for id: " << pointId << std::endl;
	...
	// Get nearby points
	std::vector<int> nearbyPoints = tree->search(points[pointId], distanceTol);
	...
	for (int id : nearbyPoints)
	{
		...
		if (isProcessed[id] == false)
		{
			...
			proximity(points, tree, distanceTol, id, cluster, isProcessed);
		}			
	}
}
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	for (int i = 0; i < points.size(); i++)
	{
		...
		if (isProcessed[i] == false)
		{
			...
			// Call Proximity
			proximity(points, tree, distanceTol, i, newCluster, isProcessed);
